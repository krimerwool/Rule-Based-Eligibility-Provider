...
def get_rule():
    ...
    number_of_rules = int(request.args.get('number_of_rules'))  # Ensure this is an integer
...
def select_rule():
    ...
    return jsonify({'selected_rule_number': rule_number})
...
def add_rule():
    ...
    number_of_rules += 1
    number_of_rules_to_combine = int(request.args.get('number'))  # Ensure this is an integer
    ...
    for i in range(number_of_rules_to_combine):
        rule_number = request.args.get(f"rule_number_{i+1}")  # Get individual rule numbers
        list_of_rules_to_combine.append(rule_number)

    combined_ast = combine_rules(list_of_rules_to_combine, select_operator)  # Use the selected operator
